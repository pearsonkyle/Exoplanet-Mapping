...
if __name__ == "__main__":
    # placeholder for loading pickle object
    ...
    for t in stargets:
        ...
        for f in filters:
            ...
            # global binning data
            im = ax[ai].scatter(bphase-0.25, bflux+offset, marker='.',cmap='jet',c=[temps[t]]*len(bflux),vmin=500,vmax=3000, alpha=0.5, zorder=2)
            ...
            for e in epochs:
                ...
                if "GJ" in t:
                    ax[ai].plot(phase[si]-0.25, data['aper_transit'][si]+offset, ls='-', color='white', alpha=0.5, zorder=3)
                else:
                    ax[ai].plot(phase[si]-0.25, data['aper_transit'][si]+offset, ls='-', color='black', alpha=0.5, zorder=3)
    ...
    ax[0].set_ylim([1-0.03,1+offset-0.02])
    ax[1].set_ylim([1-0.03,1+offset-0.02])
    ...
    plt.savefig("FIGURES/phasecurves.png")
    #plt.show()
