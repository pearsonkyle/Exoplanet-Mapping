...
def gaussian_weights(X, w=None, neighbors=50):
    ...
    for point in range(X.shape[0]):
        ...
        nearest[point,:] = ind
    gw[np.isnan(gw)] = 1./neighbors
    gw[np.isinf(gw)] = 1./neighbors
    return gw, nearest.astype(int)

def engineer_features(params: np.ndarray) -> np.ndarray:
    # unpacking the parameter values
    params = params.T
    o_x, o_y, w_x, w_y, s_x, s_y, k_x, k_y = params
    # Create a matrix of the independent variables
    ndim = params.shape[0]

    n_coeff = 53
    X = np.zeros((len(o_x), n_coeff))
    ...
    X[:, 0] = 1 # reset the first column to 1

    return X

def decorrelate_heavy(signal: np.ndarray, params: np.ndarray, verbose: bool = False) -> tuple:
    """
    Function that heavily decorrelates a signal based on a given set of parameters.
    
    This function uses a linear least-squares regression to decorrelate a signal 
    based on a set of parameters. The decorrelation is done by dividing the signal 
    by the model that is computed using the parameters. 
    The model is computed using the following equation:
    
    model = k0 * 1 + k1 * o_x + k2 * o_y + k3 * w_x + k4 * w_y + k5 * s_x + k6 * s_y + k7 * k_x + k8 * k_y + 
            k9 * o_x**2 + k10 * o_y**2 + k11 * w_x**2 + k12 * w_y**2 + k13 * s_x**2 + k14 * s_y**2 + k15 * k_x**2 + k16 * k_y**2 + 
            k17 * o_x * o_y + k18 * o_x * w_x + k19 * o_x * w_y + k20 * o_x * s_x + k21 * o_x * s_y + k22 * o_x * k_x + k23 * o_x * k_y + 
            k24 * o_y * w_x + k25 * o_y * w_y + k26 * o_y * s_x + k27 * o_y * s_y + k28 * o_y * k_x + k29 * o_y * k_y +
            k30 * w_x * w_y + k31 * w_x * s_x + k32 * w_x * s_y + k33 * w_x * k_x + k34 * w_x * k_y +
            k35 * w_y * s_x + k36 * w_y * s_y + k37 * w_y * k_x + k38 * w_y * k_y +
            k39 * s_x * s_y + k40 * s_x * k_x + k41 * s_x * k_y +
            k42 * s_y * k_x + k43 * s_y * k_y +
            k44 * k_x * k_y + 
            k45 * o_x**3 + k46 * o_y**3 + k47 * w_x**3 + k48 * w_y**3 + k49 * s_x**3 + k50 * s_y**3 + k51 * k_x**3 + k52 * k_y**3
        
    where:
        k0...k52 are the regression coefficients
        o_x, o_y, w_x, w_y, s_x, s_y, k_x, k_y are the decorrelation parameters
    
    The decorrelated signal is returned along with the model and the regression 
    coefficients. 
    
    Parameters
    ----------
    signal : np.ndarray
        The signal to decorrelate.
    params : np.ndarray
        The decorrelation parameters.
    verbose : bool
        If True, print the regression coefficients to the screen.
    
    Returns
    -------
    tuple
        A tuple containing the decorrelated signal, the model, and the 
    ...
    coeffs = np.linalg.lstsq(X, signal, rcond=None)[0]

    if verbose:
        print(f'Fit coeffs: ')
        print([f'k{i} = {coeffs[i]:.3f}' for i in range(n_coeff)])

    # Compute the model
    model = np.dot(X, coeffs)

    # Divide the signal by the model
    decorrelated_signal = signal / model

    return decorrelated_signal, model, coeffs


def decorrelate_light(signal: np.ndarray, X: np.ndarray, verbose: bool = False) -> tuple:
    """
    Function that heavily decorrelates a signal based on a given set of parameters.
...
class pc_fitter():
    ...
    def fit_nested(self):
        ...
        def loglike(pars):
            ...
            detrended = self.data/lightcurve
            #decorrelated_signal, wf, coeffs = decorrelate_heavy(detrended, self.syspars)
            decorrelated_signal, wf, coeffs = decorrelate_light(detrended, self.syspars)
    ...
    def create_fit_variables(self):
        ...
        #decorrelated_signal, self.wf, coeffs = decorrelate_heavy(detrended, self.syspars)
        decorrelated_signal, self.wf, coeffs = decorrelate_light(detrended, self.syspars)
        ...
        for f in photometry.keys():
            ...
            for p in priors['planets']:
                ...
                for e in events:
                    ...
                    if args.eclipse:
                        savedir = os.path.join(dirs[i], f, str(int(tmid))+f"_eclipse_ariel",)
                    elif args.transit:
                        savedir = os.path.join(dirs[i], f, str(int(tmid))+f"_transit_ariel",)
                    else:
                        savedir = os.path.join(dirs[i], f, str(int(tmid))+f"_phasecurve_ariel",)
                    ...
                    # get the decorrelation parameters
                    moments = photometry[f]['aper_moments'][mask][smask][:,1:]

                    # sigma clip moments
                    try:
                        for j in range(moments.shape[1]):
                            moments[:,j], _ = sigma_clip(moments[:,j], ndt, iterations=1)
                    except:
                        print("moments clipping failed")
                        pass

                    # height, x, y, width_x, width_y, skew_x, skew_y, kurt_x, kurt_y

                    # remove all nan values + format for fitting
                    nanmask = np.isnan(time) | np.isnan(flux) | np.isnan(npp) | np.isnan(wx) | np.isnan(wy)

                    for j in range(moments.shape[1]):
                        nanmask = nanmask | np.isnan(moments[:,j])
                    ...
                    # engineer features for instrumental systematics
                    features = engineer_features(moments)

                    # replace syspars with moments
                    syspars[:,0] = moments[:,1] # x-centroid
                    syspars[:,1] = moments[:,0] # y-centroid
                    syspars[:,2] = (moments[:,2]**2+moments[:,3]**2)**0.5 # avg stdev
                    ...
                    ax[1].plot(time, syspars[:,0], 'r.',label='x-cent')
                    ax[1].plot(time, syspars[:,1], 'b.',label='y-cent')
                    ax[1].set_ylabel('Pixel')
                    ax[1].set_xlabel("Time [BJD]")
                    model = phasecurve(time, tpars)
                    detrended = flux/model
                    #decorrelated_signal, wf, coeffs = decorrelate_heavy(detrended, moments)
                    decorrelated_signal, wf, coeffs = decorrelate_light(detrended, features)
                    ...
                    myfit = pc_fitter(time, flux, flux_err, tpars, mybounds, features, mode='ns', verbose=True)
                    ...
                    ax[0].set_xlabel("Time [BJD]")
                    ax[0].set_ylabel("Flux [MJy/sr]")
                    ax[1].plot(time, syspars[:,0], 'r.',label='x-cent',alpha=0.5)
                    ax[1].plot(time, syspars[:,1], 'b.',label='y-cent',alpha=0.5)
                    ax[1].set_ylabel('Pixel')
                    ax[1].set_xlabel("Time [BJD]")
                    ax[2].plot(time, syspars[:,2], 'k.',alpha=0.5)
                    ax[2].set_ylabel('Average Stdev [px]')
                    ax[2].set_xlabel("Time [BJD]")
